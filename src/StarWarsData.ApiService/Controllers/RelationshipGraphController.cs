...
public class RelationshipGraphController : ControllerBase
{
    ...
    public async Task<List<string>> GetEntityTypes(CancellationToken ct)
    {
        var types = await _nodes.DistinctAsync(
            ...
            FilterDefinition<GraphNode>.Empty, cancellationToken: ct);
        ...
        CancellationToken ct = default)
    {
        ...
        var allEdges = new List<(int from, string fromName, int to, string toName, string label, double weight)>();
        ...
        var nodeDocs = await _nodes.Find(Builders<GraphNode>.Filter.In(n => n.PageId, nodeIds))
            .ToListAsync(ct);
