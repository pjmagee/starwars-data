...
public class GalaxyMapReadService(IMongoClient mongoClient, IOptions<SettingsOptions> settings)
{
    readonly IMongoCollection<GalaxyYearDocument> _yearDocs = mongoClient
        ...
        .GetCollection<GalaxyYearDocument>(Collections.GalaxyYears);
    readonly IMongoCollection<GalaxyOverviewDocument> _overviewColl = mongoClient
        .GetDatabase(settings.Value.DatabaseName)
        .GetCollection<GalaxyOverviewDocument>(Collections.GalaxyYears);
    ...
    public async Task<List<GalaxyYearDocument>> GetYearRangeAsync(
        int fromYear,
        int toYear,
    ...
    )
    {
        return await _yearDocs
            .Find(
                Builders<GalaxyYearDocument>.Filter.And(
    ...
    public async Task<Dictionary<string, TerritoryFactionInfo>> GetFactionInfoAsync(
        CancellationToken ct = default
    )
    {
        var overview = await GetOverviewAsync(ct);
        return overview?.Factions.ToDictionary(
                f => f.Name,
                f => f,
