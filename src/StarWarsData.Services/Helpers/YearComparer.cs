...
public class YearComparer : Comparer<string>
{
    ...
    public override int Compare(string? x, string? y)
    {
        ...
        if (
            x.Contains(Bby, StringComparison.OrdinalIgnoreCase)
            && y.Contains(Aby, StringComparison.OrdinalIgnoreCase)
        )
            return -1;
        if (
            x.Contains(Aby, StringComparison.OrdinalIgnoreCase)
            && y.Contains(Bby, StringComparison.OrdinalIgnoreCase)
        )
            return 1;

        var xDemarcation = x.Contains(Bby, StringComparison.OrdinalIgnoreCase)
            ? Demarcation.Bby
            : Demarcation.Aby;
        var yDemarcation = y.Contains(Bby, StringComparison.OrdinalIgnoreCase)
            ? Demarcation.Bby
            : Demarcation.Aby;
