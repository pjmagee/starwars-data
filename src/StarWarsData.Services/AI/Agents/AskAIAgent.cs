...
)
{
    ...
    /// <summary>
    /// Render the AskAI agent's system prompt for the given database name.
    /// Public so the test fixture (and any future evaluation harness) can use the same
    /// prompt the production agent does without spinning up the full agent.
    ///
