...
public class GraphRAGToolkit
{
    ...
    )
    {
        var filters = new List<FilterDefinition<RelationshipCrawlState>>
        {
            ...
            Builders<RelationshipCrawlState>.Filter.Regex(s => s.Name, new BsonRegularExpression(query, "i")),
        };
    ...
    public async Task<string> GetEntityRelationships(
        [Description("The PageId of the entity (from search_graph_entities)")]
        int entityId,
        [Description("Optional comma-separated labels to filter (e.g. 'parent_of,trained_by'). Omit for all.")]
        string? labelFilter = null,
        [Description("Optional continuity filter: Canon, Legends, or omit for all")]
        string? continuity = null,
        [Description("Max edges to return (default 30)")]
        int limit = 30
    )
    {
        ...
        if (edges.Count == 0)
            return JsonSerializer.Serialize(new { entityId, relationships = new { }, totalEdges = 0, note = "No relationships found. The entity may not have been processed yet." });
    ...
    )
    {
        ...
        return JsonSerializer.Serialize(results.Select(d =>
        {
    ...
    public async Task<string> TraverseGraph(
        ...
        int entityId,
        [Description("Optional comma-separated labels to follow (e.g. 'trained_by,trained'). Omit for all.")]
        string? labels = null,
    ...
    )
    {
        ...
        var edgeList = new List<(int from, string fromName, int to, string toName, string label, double weight, string evidence, int depth)>();
            ...
            if (doc.Contains("network"))
            {
                foreach (var netDoc in doc["network"].AsBsonArray.OfType<BsonDocument>())
                {
                    ...
                    if (edgeDedup.Add((nFromId, nToId, nLabel)))
                        edgeList.Add((nFromId, nFromName, nToId, nToName, nLabel, nWeight, nEvidence, depth));
                }
            }
        }
    ...
    public async Task<string> FindConnections(
        [Description("PageId of the first entity")]
        int entityId1,
        [Description("PageId of the second entity")]
        int entityId2,
        [Description("Maximum path length to search (default 3, max 4)")]
        int maxHops = 3,
        [Description("Optional continuity filter: Canon, Legends, or omit for all")]
        string? continuity = null
    )
    {
        ...
        if (entityId1 == entityId2)
            return JsonSerializer.Serialize(new { connected = true, pathLength = 0, path = Array.Empty<object>(), note = "Same entity" });
        ...
        var pathFromE2 = new List<(int from, int to, string label, string evidence, string fromName, string toName)>();
        ...
        foreach (var step in pathFromE1)
            path.Add(new { from = step.fromName, to = step.toName, label = step.label, evidence = Truncate(step.evidence, 200) });
    ...
    public async Task<string> SearchChunks(
        [Description("Natural language search query (e.g. 'Battle of Endor aftermath', 'Darth Vader's redemption')")]
        string query,
        [Description("Optional entity type filter (e.g. Character, Planet, Battle). Omit for all types.")]
        string? type = null,
        [Description("Optional continuity filter: Canon, Legends, or omit for all")]
        string? continuity = null,
