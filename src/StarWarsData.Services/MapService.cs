...
public class MapService
{
        ...
        IMongoClient mongoClient)
    {
        ...
        _db = mongoClient.GetDatabase(_settingsOptions.RawDb);
    }
    ...
    public async Task<IEnumerable<GalaxyMapItem>> GetPlanets()
    {
        ...
        var col = _db.GetCollection<InfoboxRecord>("CelestialBody");
        // Only include grid-enabled bodies that are terrestrial planets
        var gridFilter = Builders<InfoboxRecord>.Filter.ElemMatch(r => r.Data, d => d.Label == "Grid square");
        var classFilter = Builders<InfoboxRecord>.Filter.ElemMatch(r => r.Data,
            d => d.Label == "Class" && d.Values.Contains("Terrestrial"));
        var filter = Builders<InfoboxRecord>.Filter.And(gridFilter, classFilter);
    ...
    {
        var col = _db.GetCollection<InfoboxRecord>("Sector");
    ...
    {
        var col = _db.GetCollection<InfoboxRecord>("Region");
        var recs = await col.Find(_ => true).ToListAsync();
    ...
    {
        var col = _db.GetCollection<InfoboxRecord>("System");
        var rec = await col.Find(r => r.PageId == id).FirstOrDefaultAsync();
        if (rec == null) return null;
