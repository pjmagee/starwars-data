...
)
{
    ...
    readonly IMongoCollection<TerritoryYearDocument> _yearDocs = mongoClient
        .GetDatabase(settings.Value.DatabaseName)
        .GetCollection<TerritoryYearDocument>(Collections.TerritoryYears);
    ...
    public async Task InferTerritoryAsync(CancellationToken ct = default)
    {
        ...
        logger.LogInformation(
            "Territory inference: {Govs} govs, {Planets} planet-regions, {Factions} factions",
            govWithDates.Count,
        ...
        logger.LogInformation(
            "Territory inference: {Count} Canon event nodes with dates",
            eventNodes.Count
        );
        ...
        foreach (var doc in gridPages)
        {
            ...
            {
                if (item.GetValue("Label", "").AsString != "Grid square")
                    continue;
                var vals =
        ...
        logger.LogInformation(
            "Territory inference: {Count} planets with grid coordinates",
            planetToGrid.Count
        );
        ...
        logger.LogInformation(
            "Territory inference: built adjacency list from {Count} Canon edges",
            allCanonEdges.Count
        );
