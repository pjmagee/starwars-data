...
public class InfoboxDownloader
{
    ...
    // Removed single collection; dynamic per template
        ...
        HttpClient httpClient)
    {
        ...
        _rawDb = mongoClient.GetDatabase(_settingsOptions.RawDb);
    ...
    public async Task ExecuteAsync(CancellationToken token)
    {
        do
        {
            ...
            {
                var wikiPages = pagesWithPropElement.EnumerateArray().Where(x => x.GetProperty("ns").GetInt32().Equals(_settingsOptions.PageNamespace)).ToList();
    ...
    {
        InfoboxRecord record = new InfoboxRecord();
                ...
                if (infobox.ValueKind == JsonValueKind.Object && infobox.TryGetProperty("data", out var data) && data.ValueKind == JsonValueKind.Array)
                {
                    foreach (var item in data.EnumerateArray())
                    {
                        if (item.TryGetProperty("type", out var typeProperty) && item.TryGetProperty("data", out var dataProperty) && typeProperty.ValueKind == JsonValueKind.String)
                        {
                            ...
                            else if (typeProperty.ValueEquals("group") && dataProperty.TryGetProperty("value", out var groupDataValue) &&
    ...
    async Task<DataValue> GetDataValue(string html)
    {
        ...
        return new DataValue
        {
            Values = doc.Body?
                ...
                .Select(static x => x.Trim(TrimValues)).Where(static line => !string.IsNullOrWhiteSpace(line)).Distinct().ToList() ?? new List<string>(),
    ...
    async Task Save(InfoboxRecord record, CancellationToken token)
    {
        try
        {
            var collection = _rawDb.GetCollection<InfoboxRecord>(record.Template);
            var filter = Builders<InfoboxRecord>.Filter.Eq(x => x.PageId, record.PageId);
            var options = new ReplaceOptions { IsUpsert = true };
            await collection.ReplaceOneAsync(filter, record, options, token);

            _logger.LogInformation("Saved InfoboxRecord for PageId {PageId} - {PageTitle}", record.PageId, record.PageTitle);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Failed to save InfoboxRecord with PageId {PageId}: {ErrorMessage}", record.PageId, ex.Message);
        }
    }
}
