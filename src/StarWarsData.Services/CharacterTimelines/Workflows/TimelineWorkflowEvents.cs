...
internal sealed record PageDiscoveredData(int PageId, string Title, string WikiUrl, string? Template, string Continuity, string Relationship); // "self", "incoming", "outgoing"
...
internal sealed record DiscoveryCompleteData(int TotalPages, int IncomingLinks, int OutgoingLinks, int KgLinks);

/// <summary>
/// Emitted by BatchExtractionExecutor for each event found during extraction.
/// </summary>
internal sealed class EventExtractedEvent(EventExtractedData data) : WorkflowEvent(data);

internal sealed record EventExtractedData(string EventType, string Description, float? Year, string? Demarcation, string SourcePageTitle);
...
internal sealed record BundlingCompleteData(int TotalPages, int BatchCount, List<int> BatchSizes);
...
internal sealed record BatchExtractionStartedData(int BatchIndex, int TotalBatches, int PageCount, List<string> PageTitles);
...
internal sealed record BatchExtractionEmptyData(int BatchIndex, int PageCount);
...
internal sealed record BatchExtractionFailedData(int BatchIndex, int PageCount, string Error);
...
internal sealed record ConsolidationCompleteData(int InputEventCount, int OutputEventCount, int DuplicatesRemoved);
...
internal sealed record ReviewCompleteData(int InputEventCount, int OutputEventCount, int EventsRemoved);
