// D3.js Galactic Map V2 - Drill-down interaction
...
export function initialize(containerId, overview, dotNetRef) {
    ...
    overview.regions.forEach((region) => {
        ...
        // Label at centroid of region cells
        ...
        rg.append('text')
            ...
            .attr('fill-opacity', 0.5).attr('font-size', '13px').attr('font-weight', '600')
    ...
    function renderRegionSystems(systems, region) {
        ...
        // Spread systems across their cell with force layout.
        // Use a collision radius proportional to how dense each cell is.
        const cellCounts = {};
        filtered.forEach(s => { const k = `${s.col},${s.row}`; cellCounts[k] = (cellCounts[k] || 0) + 1; });

        const nodes = filtered.map(sys => ({
            ...sys,
            x: sys.col * cellW + cellW / 2 + (Math.random() - 0.5) * cellW * 0.7,
            y: sys.row * cellH + cellH / 2 + (Math.random() - 0.5) * cellH * 0.7,
        }));

        // Larger collision radius to prevent overlap, weaker centering so nodes can spread
        const collideR = Math.max(cellW * 0.035, cellW * 0.015 * Math.sqrt(filtered.length / Math.max(1, Object.keys(cellCounts).length)));
        const sim = d3.forceSimulation(nodes)
            .force('x', d3.forceX(d => d.col * cellW + cellW / 2).strength(0.12))
            .force('y', d3.forceY(d => d.row * cellH + cellH / 2).strength(0.12))
    ...
    function renderCellSystems(systems, col, row) {
        ...
        // Force-directed layout within the cell.
        // Spread across the full cell area and use a collision radius that
        // accounts for the label text width to prevent overlap.
        ...
        // Scale collision radius based on density — more systems = let them spread further
        const density = filtered.length;
        const collideR = density > 30
            ? cellW * 0.03
            : density > 10
                ? cellW * 0.05
                : cellW * 0.06;
        ...
        const sim = d3.forceSimulation(nodes)
            .force('x', d3.forceX(cx).strength(density > 30 ? 0.03 : 0.08))
            .force('y', d3.forceY(cy).strength(density > 30 ? 0.03 : 0.08))
            .force('collide', d3.forceCollide(collideR).strength(0.9).iterations(4))
            .stop();
        // More iterations for denser cells
        const iters = density > 30 ? 200 : 120;
        for (let i = 0; i < iters; i++) sim.tick();

        // Clamp nodes to stay within cell boundaries (with padding)
        const pad = cellW * 0.05;
        const xMin = cx - cellW / 2 + pad, xMax = cx + cellW / 2 - pad;
        const yMin = cy - cellH / 2 + pad, yMax = cy + cellH / 2 - pad;
        nodes.forEach(n => {
