...
- [x] T033 [P] [US2] Vendor family-chart-premium per [quickstart.md § 1](./quickstart.md#1-vendor-family-chart-premium). **Deviation from spec**: the upstream GitHub repo (donatso/family-chart-premium) publishes only README/LICENSE/package.json — no `dist/` directory. The actual bundle is shipped via npm. Pulled via `npm pack family-chart-premium@beta` (tarball shasum `494492bc21c9c6ef3da631b05aebcc1e404dee35`, npm version `0.0.0-beta.2`). Vendored `dist/family-chart.js` + `dist/styles/*.css` + `LICENSE.txt` into `src/StarWarsData.Frontend/wwwroot/lib/family-chart-premium/`. `VERSION.txt` records both the npm tarball shasum AND the upstream commit ref `cbd4a41e23892c58b6b86e87cf4f9ce1ce6de4f0`. Principle I.
- [x] T034 [US2] Added `<link rel="stylesheet" href="lib/family-chart-premium/styles/family-chart.css" />` and `<script src="lib/family-chart-premium/family-chart.js"></script>` to `App.razor` alongside the existing d3 reference.
- [x] T035 [P] [US2] Created `src/StarWarsData.Frontend/wwwroot/js/family-tree.js`. Uses `window.f3.createChart`, opts into `SpouseLinkTextPlugin` and `KinshipPlugin` via `chart.use(new f3.SpouseLinkTextPlugin({…}))` / `chart.use(new f3.KinshipPlugin())` (the exposed class names — verified by reading `dist/types/index.d.ts`). The README's `f3.plugins.*` shorthand isn't on the public surface; the canonical API is `chart.use(new ClassName(options))`. Wires `setOnCardClick((e, td) => dotNetRef.invokeMethodAsync('OnPersonClicked', td.data.data.pageId))`. No `.editTree()` call. Read-only.
- [x] T036 [P] [US2] Created `src/StarWarsData.Frontend/Components/Shared/FamilyTreeView.razor`. Sibling of `AskGraphView.razor`. MudPaper wrapper, continuity badge (`Color.Primary` for Canon, `Color.Secondary` for Legends, `Color.Default` otherwise), advisory `MudAlert` chip when any limitation flag is set, `<div id="family-tree-{guid}" class="f3" style="width:100%;height:720px;">` mount.
- [x] T037 [US2] Mobile fallback (`d-block d-md-none`) rendering `MobileSummary` via `MudMarkdown` per Design-011 (inline with T036).
- [x] T038 [US2] `IAsyncDisposable`, `DotNetObjectReference<FamilyTreeView> _self`, `[JSInvokable] OnPersonClicked(int pageId)` → `Nav.NavigateTo($"/knowledge-graph/nodes/{pageId}")`. Synthetic stubs already carry their real PageId per data-model.md § Step 5.
- [x] T039 [US2] `JS.InvokeAsync<IJSObjectReference>("import", "./js/family-tree.js")` in `OnAfterRenderAsync(firstRender)`. DisposeAsync releases both `_module` (with `JSDisconnectedException` swallow) and `_self`.
- [x] T040 [US2] Subscribed `FamilyTreeView` to `GlobalFilterService.OnChange`. The Descriptor's explicit continuity takes precedence; falls through to the global filter when null. Realm always comes from the global filter.
- [x] T041 [US2] Added `FamilyTreeDescriptor? FamilyTreeDescriptor` field to `ConversationMessage`, added the `case "family_tree" when msg.FamilyTreeDescriptor is not null` branch in the visualization switch, added the `"render_family_tree" => "family_tree"` mapping in the vizType switch, and added the descriptor deserialization branch in `OnFunctionResult` plus session save/load (`BuildSessionMessages` and `LoadSession`).

**Mid-task discovery**: the premium build emits an "ADD" empty-slot card with an editing affordance whenever a relative reference is unresolved. Spec says read-only. Suppressed via two rules in `wwwroot/style.css`:
`div.f3 .card.card-to-add { display:none !important; }` and `div.f3 .f3-to-add-label { display:none !important; }`. These are scoped to the JS-managed container so they cannot leak into other components.
...
- [x] T047 Chrome DevTools MCP loop done. Skywalker prompt → agent picked `render_family_tree` (verified: tool group shows "Used 3 tools: Search entities, Get relationship types, Render family tree"). Endpoint call `GET /api/RelationshipGraph/family-tree/452390?*` → **200 in 310ms** (fresh) / **151ms** (session reload). Console: zero errors. Screenshot saved to [screenshots/skywalker-tree-desktop.png](./screenshots/skywalker-tree-desktop.png). Continuity badge "Canon" rendered with `Color.Primary` per Principle VII.
- [x] T048 [P] Mobile fallback verified at 414×896. Chart `<div>` is absent from the DOM at this viewport (`d-md-block`); `MobileSummary` MudMarkdown bullets rendered ("Centered on Anakin Skywalker; the tree expands 3 generations…", 5 bullets total). Console: zero errors at mobile size. Screenshot at [screenshots/skywalker-tree-mobile.png](./screenshots/skywalker-tree-mobile.png).
- [x] T049 [P] Watermark probe done. Free build draws a "Family Chart (Free)" badge in the bottom-right of the chart canvas. **Kept as-is per Principle I + upstream restriction #2** — not patched. Full writeup at [screenshots/watermark-probe.md](./screenshots/watermark-probe.md). No license-key modal, no "Powered by …" footer outside the chart.
- [x] T050 [P] Cycle probe done. "Show me Boba Fett's family tree" rendered cleanly — Boba (selected) + Jango Fett (parent, modelling clone lineage) with no infinite loop, no stack overflow, zero console errors after render. The documented fallback (`render_graph` Tree mode) was NOT triggered. Full writeup at [screenshots/cycle-probe.md](./screenshots/cycle-probe.md). Screenshot at [screenshots/boba-fett-tree.png](./screenshots/boba-fett-tree.png).
