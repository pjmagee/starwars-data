...
- MongoDB.Driver — read against `kg.nodes`/`kg.edges` only. No `raw.*` reads at runtime (Principle VI).
...
- **KG-First** (Principle VI). All data — including the `Gender` slot — comes from `kg.nodes`/`kg.edges`. `"Gender"` is registered in `FieldSemantics.Properties` so the generic NodeBuilder loop projects it onto `kg.nodes.properties["Gender"]` for every Character. The family-chart constraint that `gender` must be `"M"|"F"` is enforced inside `BuildFamilyTreeAsync`'s `ReadGender(GraphNode)` and never propagates outside the projection. Missing / non-binary values default to `"M"` with the PageId appended to `Limitations.MissingGenders` — that's a library constraint, NOT a data-availability gap. See [research.md R-5](./research.md#r-5-gender-field-source-of-truth-kg-first-principle-vi-gap) for the correction that retracted the original "documented soft-edge" framing.
...
| **II. Production Data Safety (NON-NEGOTIABLE)** | PASS | All reads target `kg.nodes`/`kg.edges` in the default DB (`starwars-dev`). No writes. No `raw.*` reads. Integration tests use Testcontainers MongoDB — never live prod. Agent tests use `starwars-dev`. |
...
| **VI. KG-First Data Access at Runtime** | PASS | All data — including the `Gender` slot — comes from `kg.nodes`/`kg.edges`. `"Gender"` is registered in `FieldSemantics.Properties` so the generic NodeBuilder loop projects it onto `kg.nodes.properties["Gender"]` for every Character. `BuildFamilyTreeAsync` reads `node.Properties["Gender"]` inline during the BFS-result projection — no extra round-trip, no `raw.*` access. The family-chart constraint that gender be `"M"` or `"F"` is enforced inside `ReadGender(GraphNode)` and never leaks. Missing / non-binary values default to `"M"` with the PageId appended to `Limitations.MissingGenders` — that's a library constraint, NOT a Principle VI gap. **Earlier draft of this plan (and the original kg-expert implementation) read from `raw.pages.infobox.Data` based on a wrong claim in research.md R-5; both have been corrected.** |
...
**Re-evaluation after Phase 1**: passes — no new violations surface in data-model / contracts. The "Gender source-of-truth gap" called out in the earlier draft was a misreading; `kg.nodes.properties["Gender"]` is populated by the standard NodeBuilderBase loop (corrected in research.md R-5 + the Principle VI row above).
