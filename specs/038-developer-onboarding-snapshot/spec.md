...
[eng/docs/maintainer-snapshot-publish.md](../../eng/docs/maintainer-snapshot-publish.md).
**Date:** 2026-05-19
**Author:** Patrick Magee + Claude
**Companion docs:** [Design-033](../033-prod-to-dev-data-refresh/spec.md) (raw-only,
recurring QA refresh — the *opposite* trade-off), [Design-010](../010-mongodb-migration-environment-promotion/spec.md),
[ADR-005](../../eng/adr/005-mongodb-migration-strategy.md)
...
1. ~~**Should Aspire fully manage the local Mongo container?**~~ **Resolved —
   yes, opt-in** ([ADR-010](../../eng/adr/010-aspire-managed-dev-mongo.md)). A
   ...
   snapshot-restore is gated on the same flag so it can never clobber a shared
   server's `starwars-dev`. Compile-verified; one real opt-in `aspire run`
   still required (see Validation status).
