...
3. **Front-line shading between factions.** Cells adjacent to two contested factions get a hash-pattern tint. Computed from `galaxy.years` region control + grid adjacency. Already half-implemented (the `contested` flag exists); just needs a per-cell rather than per-region render. Cheap extension of the existing territory layer.
