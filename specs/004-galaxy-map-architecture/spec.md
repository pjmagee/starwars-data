...
All endpoints are internal (not internet-exposed); the Frontend calls them via the authenticated `HttpClient` with an `X-User-Id` header set by the `DelegatingHandler` (see [ADR 001](../../eng/adr/001-internal-api-auth.md)).
...
- Edge temporal bounds are mostly derived (lifespan overlap), not explicit — see [001-temporal-facets.md](../001-temporal-facets/spec.md) for the `TemporalExplicit` flag proposal.
