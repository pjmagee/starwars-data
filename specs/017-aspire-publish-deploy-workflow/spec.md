...
**Related:** [ADR-005 MongoDB Migration Strategy](../../eng/adr/005-mongodb-migration-strategy.md), [Design-010 MongoDB Migration Environment Promotion](../010-mongodb-migration-environment-promotion/spec.md), [CLAUDE.md (Build & Run section)](../../CLAUDE.md)
...
3. `aspire publish -e Production` — emits `docker-compose.yaml` + an **unfilled** `.env.Production` template into `./aspire-output/`. The published artifact contains placeholders only; the deploy host materializes real values from its own secret store. A grep guard runs after publish and fails the build if the artifact ever contains text matching a live credential pattern, as a forward-compatibility check against changes in Aspire's `publish` contract.
4. Uploads the `release` artifact (compose + template) and pushes a `vX.Y.Z` git tag.

The deploy host runs `docker compose --env-file .env.Production up -d` after filling the template. CI must use `aspire publish` (not `prepare-starwars` or `deploy`) for any step that produces an uploaded artifact, so the artifact stays free of resolved parameter values.
