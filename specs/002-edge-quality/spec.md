...
- [ADR-003: KG Query Architecture](../../eng/adr/003-kg-query-architecture.md) — `(fromId, toId, label)` unique constraint, ETL dedup step, denormalization of `fromRealm`/`toRealm`/`reverseLabel` on edges
- [Design-007: KG Bidirectional Edges View](../007-kg-bidirectional-edges-view/spec.md) — `reverseLabel` denormalization and the `kg.edges.bidir` view that builds on this work
