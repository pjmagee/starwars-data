...
Status: Superseded by [Design-024 — Typed node builders](../024-typed-node-builders/spec.md). The outcome shipped (2026-04-28, `ed5d53f246`): `has_role`/`held_by` registered in `FieldSemantics` and the Character→TitleOrPosition promotion implemented — but via Design-024's `CharacterNodeBuilder.OnFinalize` per-type override, **not** the `InfoboxGraphService.cs:211` drop-filter flip proposed here (that filter was deliberately left intact). The standalone `0017-titles-to-has-role-edges.js` migration was never created; the typed-builder rebuild handles it. Treat Design-024 as authoritative.
Date: 2026-04-28
Author: Patrick Magee
Cross-refs: [Design-001 — Temporal facets](../001-temporal-facets/spec.md), [Design-035 — KG per-type builders](../035-kg-per-type-builders/spec.md), [Design-018 — KG enrichments architecture](../018-kg-enrichments-architecture/spec.md), [Design-021 — Edge bound provenance](../021-edge-bound-provenance/spec.md)
...
- [Design-021 — Edge bound provenance](../021-edge-bound-provenance/spec.md) — `has_role` inherits the BoundsSource provenance model.
- [Design-035 — KG per-type builders](../035-kg-per-type-builders/spec.md) — character node builder is where `Titles` extraction logic lives.
