...
**Related:** [Design-003 AI Game Master](../003-ai-game-master/spec.md), [Design-015 MongoDB GraphRAG Context Provider](../015-mongodb-graphrag-context-provider/spec.md), [ADR-001 Internal API Auth](../../eng/adr/001-internal-api-auth.md), [CLAUDE.md (GDPR section)](../../CLAUDE.md)

**Scope boundary:** This doc is about *persistent, per-user, write-back memory* the agent **learns from conversations**. Read-only retrieval over the curated universe KG (canon Wookieepedia data, identical for every user) is the inverse problem and lives in [Design-015](../015-mongodb-graphrag-context-provider/spec.md).
...
Per [Design-015's "One graph, not two"](../015-mongodb-graphrag-context-provider/spec.md#one-graph-not-two) we explicitly avoided forking the universe KG. **This design adds a *second*, genuinely separate graph — and that separation is the point.**
