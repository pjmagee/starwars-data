...
**Related:** [Design-035 KG Per-Type Node Builders](../035-kg-per-type-builders/spec.md), [Design-013 KG Property/Edge Duality](../013-kg-property-edge-duality/spec.md), [ADR-003 KG Query Architecture](../../eng/adr/003-kg-query-architecture.md), [ADR-005 MongoDB Migration Strategy](../../eng/adr/005-mongodb-migration-strategy.md)
...
7. **Stages E1–E9** (per-consumer migration PRs) ⏳ in progress —
   - **Stage E1** ⏳ in progress — `KnowledgeGraphQueryService` swaps to `kg.nodes.enriched` + `kg.edges.enriched`, and the `/knowledge-graph` page renders Phase 1 vs Phase 2 with distinct chip/row styling. UI provenance pattern documented in [Design-019](../019-kg-enrichment-ui-provenance/spec.md) and reused by E2–E9.
