...
**Status:** Superseded — pivot rejected with [Design-025](../025-holocron-tool-using-agent/spec.md). See [ADR-007](../../eng/adr/007-holocron-hardening-over-rewrite.md). The fan-out architecture below was not built; the v1.x sequential five-executor workflow was kept and a sixth stage (`HolocronEvidenceVerifierExecutor`) was inserted between consolidator and apply.
**Date:** 2026-04-29
**Author:** Patrick Magee + Claude
**Related:** [Design-018 KG enrichments architecture](../018-kg-enrichments-architecture/spec.md), [Design-020 Holocron async pipeline](../020-holocron-async-pipeline/spec.md), [Design-025 Holocron tool-using agent](../025-holocron-tool-using-agent/spec.md), [ADR-006 Long-running AI workflow pipelines](../../eng/adr/006-long-running-ai-workflow-pipelines.md), [ADR-007 Hardening over rewriting](../../eng/adr/007-holocron-hardening-over-rewrite.md)

## Outcome (added 2026-04-29)

This design was a follow-on to [Design-025](../025-holocron-tool-using-agent/spec.md). When the parent design's pivot was rejected (see [ADR-007](../../eng/adr/007-holocron-hardening-over-rewrite.md)), the orchestration choice in this document became moot — there is no tool-using agent to fan out, and the sequential workflow was demonstrably handling Anakin-class loads (~30 min wall-clock, 341 batches) with quality gains coming from the verifier and prompt rewrite, not from concurrency.
...
- **Sixth executor inserted, sequentially.** `HolocronEvidenceVerifierExecutor` (post-consolidator, pre-apply) — see [ADR-007](../../eng/adr/007-holocron-hardening-over-rewrite.md). It uses the `IChatClient` directly with structured-output verdicts, not an `AIAgentHostExecutor`.
