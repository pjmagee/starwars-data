...
`CorpusStatsService` computes one snapshot (collection counts + recent lists + last-sync timestamp) and caches it in `IMemoryCache` with a short TTL (default **5 minutes**). All public traffic in a TTL window is served from memory; at most one refresh per TTL touches MongoDB. Collection totals use `EstimatedDocumentCountAsync` (O(1) metadata read), not `CountDocumentsAsync` — exact precision is irrelevant to an "is the site alive" signal, and the estimate cannot be made to scan the corpus. Recent-item lists are bounded `Find().Sort().Limit()` queries that **must** be index-backed: `raw.pages` is sorted by `downloadedAt` (index `idx_downloadedAt`, created by `RecordService.EnsureIndexesAsync`) and `search.chunks` by `createdAt` (index `ix_createdAt`, created by `ArticleChunkingService.EnsureIndexesAsync`). Without these the queries degrade to a `COLLSCAN` + blocking in-memory sort over the whole collection — the indexes are a hard prerequisite of this design, not an optimization. The "recently chunked" feed is intentionally **per-chunk** (a flat `Find` over `ix_createdAt`), not one-row-per-page: a per-page view would require a whole-collection `$group` that no index can serve.
