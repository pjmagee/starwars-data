...
- Phase A.1 read tools (`resolve_entity` / `find_canonical_label` / `check_existing_edges` / `get_template_schema`) shipped as a foundation but were **never wired into any agent** and were later removed (issue #9) — the v1.x extractor uses the original structured-output path unchanged.
...
- **Phase A.1 read tools (Design-025 §The tool surface) shipped as a foundation but were never wired up.** Removed as unwired dead weight in issue #9 (`HolocronReadToolkit` / DTOs / tests / `ToolNames.Holocron`); the foundation now lives in git history only.
- **The synonym table is gone.** Design-025's `LabelSynonymTable` was prototyped, tested with 14 unit tests, then deleted after the user identified it as solving a problem we don't have. The later Phase A.1 `find_canonical_label` thin LINQ filter over `FieldSemantics.Relationships` was removed with the rest of the read-tool surface (issue #9).
