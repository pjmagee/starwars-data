...
- **Migrating `Ask.razor` to `AGUIChatClient`.** Resolved 2026-05-22 — pulled forward from the ≥ 2-week soak after the sidebar migration went green on the same day. `Ask.razor` now uses `IChatClient` + `ChatMessage` end-to-end; `AguiSseReader.cs` and the `AguiMessage` DTO are deleted. Renamed `Components/Shared/Agui/` is gone. Two bugs surfaced during validation: (1) empty assistant bubbles between tool calls (one per FunctionCallContent-only `ChatMessage` from `ToChatResponse()`) — fixed by skipping text-empty segments in `ApplyAssistantText`; (2) a **wire-contract asymmetry in the AGUI preview packages themselves** that kills the entire turn whenever any server-side tool fails. Evidence chain from decompiling the three packages: M.E.AI 10.5.1 `FunctionInvokingChatClient.CreateFunctionResultContent` (line 4399-4413) substitutes a plain `string` like `"Error: Function failed."` (or `Cancelled`/`NotFound`/`Unknown error` — every fallback starts with `'E'`) when a tool throws / is cancelled / fails argument binding. `Microsoft.Agents.AI.Hosting.AGUI.AspNetCore` 1.0.0-preview.260311.1 `SerializeResultContent` (line 7219-7234) then writes that string **raw** into the SSE `content` field for `TOOL_CALL_RESULT`. `Microsoft.Agents.AI.AGUI` 1.6.1-preview.260514.1 `DeserializeResultIfAvailable` (line 7197-7204) then runs `JsonSerializer.Deserialize<JsonElement>(content)` and throws `'E' is an invalid start of a value. Path: $ | LineNumber: 0 | BytePositionInLine: 0.` *inside* the streaming iterator — escaping the `await foreach` and killing the turn before the model's text can arrive. The old hand-rolled `AguiSseReader` accidentally papered over this by never parsing `content` as JSON; migrating to the typed client surfaced the latent bug. Defensive `catch (JsonException)` added in `Ask.razor.SubmitAsync` so a server-tool failure now renders a friendly "A tool returned a malformed response, so this turn couldn't complete" message instead of raw parser noise; the upstream wire-contract bug is filed as a follow-up. Rate-limit UX (precise retry-after countdown) preserved via a `RateLimitMessageHandler` that runs before `AGUIChatClient.EnsureSuccessStatusCode()` discards the body. The Sources/citation card was slimmed alongside the migration: Node (→ `/knowledge-graph/nodes/{pageId}`) + Location (galaxy map) + Wiki only; the Graph Explorer / Timeline / Holocron chips were dropped and that surface area moves to the KG node detail page they all originate from.
