...
**Related:** [Design-018 Holocron skeleton](./018-holocron-skeleton.md), [Design-020 Holocron async pipeline](./020-holocron-async-pipeline.md), [Design-021 Edge-bound provenance](./021-edge-bound-provenance.md), [Design-022 SP-4 sidebar](./022-galaxy-map-copilot.md), [ADR-006 long-running AI workflows](../adr/006-long-running-ai-workflows.md)
...
**Admin-only actions** (gated by `<AuthorizeView Roles="admin">` — same Keycloak role + dev-auth bypass used elsewhere on the Frontend; see [ADR-001](../adr/001-internal-api-auth.md)):
